...
double calcularSumaParcialPi( double *Pi, int proceso, long inicial, long terminos ) {
  ...
  *Pi += casiPi;			// Guarda el resultado en el vector y finaliza
  exit( 0 );
}


int main( int argc, char ** argv ) {
  int shmid=shmget(0xB62386,sizeof(double), 0600|IPC_CREAT);
  if(-1==shmid){
    exit(1);
  }
  double *p=(double*)shmat(shmid,NULL,0);
  ...
  for ( proceso = 0; proceso < 10; proceso++ ) {
    ...
    pid = fork();
    if ( ! pid ) {
      calcularSumaParcialPi( p, proceso, inicio, fin );
    } else {
