...
int main(int argc, char **argv) {
  float globaldata[TAMANO];/*wants to declare array this way*/
  double inicio, finalizacion;



  cout<<GREEN<<"CANTIDAD DE DATOS: "<<TAMANO<<BLACK<<endl<<endl;
  //se paraleliza el llenado del array solo si no se usa pgi o se tiene CUDA
  //# pragma acc parallel loop
  for (int i=0; i<TAMANO; i++){
    globaldata[i] = (float)(rand()%100000+1);
  }
